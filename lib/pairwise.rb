module Pairwise
  class << self
    ...
    #
    # On success returns an array of question external ids.
    def question(data)
    ...
    #
    # On success returns an array of item external ids.
    #
    # Failure on passing non-existent question id.
    ...
    #
    # state will be set to 'activated' if state is true.  Otherwise state will be set to 'suspended'.
    #
    # On response returns true.
    #
    # Failure on incorrect item id.
    def update_item_state(item_id, state)
      !send_pairwise_request("items/#{item_id}/#{state ? 'activate' : 'suspend'}", nil, 'Put').nil?
    end
    ...
    #
    # On sucess returns an array with each member a prompt external id followed by an array of item
    # external ids for that prompt.
    #
    # Failure on question_id not belonging to pairwise user or not existing
    #
    # Failure on voter_id not belonging to pairwise user or not existing
    def prompt(question_id, voter_id, n = 1, item_ids = nil, num_items = 2)
    ...
    # Create a vote on a prompt for an item or a skip if nil.  Can be cast for a voter or the
    # anonymous voter and a response time can be sent.
    #
    # prompt_id:: the prompt external id to record a vote for.
    #
    # item_id:: the item_id to record the vote for.  If item_id is nil or false a skip vote is sent.
    #
    # voter_id:: the voter external id to record the vote for.  Defaults to the anonymous voter, a value of 0.
    #
    # response_time:: the response time of the vote, default 0.
    #
    # On success returns an array of vote external ids.
    #
    # Failure on non-existent prompt_id or prompt_id not belonging to pairwise user.
    #
    # Failure on item_id not belonging to prompt.
    #
    # Failure on voter_id not beloning to current user if voter_id is greater than 0.
    ...
    #
    # On success returns an array each element being an item external id followed by a rank value.
...
private
    ...
    #
    # returns a response from the pairwise path parsed with the res_path.
    ...
    # path:: where to send the pairwise request to.
    #
    # xml:: the post body to send.  If xml is nil and method is nil (see below) a get request is sent.  Default nil.
    #
    # method:: type of request to send.  If method is nil request type is determined
    # by the value of value of xml.
